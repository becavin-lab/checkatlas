...
def clean_list_atlases_multi(client, futures, atlas_list) -> list:
    ...
    these info ['Atlas_Name','Type','Atlas_file_extension','Checkatlas_folder']
    """
...
def clean_list_atlases(atlas_list) -> list:
    """
    Go through all files and detect Seurat or Scanpy Atlas
    Then:
    - Convert Seurat files to Scanpy
    ...
    these info ['Atlas_Name','Type','Atlas_file_extension','Checkatlas_folder']
    """
    # Create dict with these info ['Atlas_Name','Type','Converted','Checkatlas_path']
    clean_atlas_dict = dict()
    for atlas_path in atlas_list:
        atlas_name = get_atlas_name(atlas_path)
        print(atlas_name)
...
def summary_table(adata, atlas_path, atlas_name, resume) -> None:
    ...
    :param atlas_path:
    :param atlas_name:
    :param resume: True if you want to run the function only if the tables does not exist already
    :return:
    """
    ...
    if resume:
        if not os.path.isfile(csv_path):
            atlas.create_summary_table(adata, csv_path, atlas_name)
    else:
        atlas.create_summary_table(adata, csv_path, atlas_name)
    ...
    if resume:
        if not os.path.isfile(csv_path):
            atlas.create_anndata_table(adata, csv_path, atlas_name)
    else:
        atlas.create_anndata_table(adata, csv_path, atlas_name)
...
def run(path, atlas_list, multithread, n_cpus):
    ...
    - Clean files list by getting only Scanpy atlas (converted from Seurat if necessary)
