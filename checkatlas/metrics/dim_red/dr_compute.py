...
def kruskal_stress(adata, key_repr):
    ...
    for key in key_repr :
        ...
        stress = np.sqrt(np.square(high_dim_distances - low_dim_distances).sum() / np.square(low_dim_distances).sum())
        print(f'dist_diff = {np.square(high_dim_distances - low_dim_distances).sum()}')
