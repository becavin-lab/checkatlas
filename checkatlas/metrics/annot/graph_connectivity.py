...
def run(adata, neighbors_key="neighbors", label_key=None, n_jobs=-1, verbose=True):
    ...
    if neighbors_key not in adata.uns:
        print(
            f"Neighbors key '{neighbors_key}' not found in adata.uns. Calculating neighbors..."
        )
        ...
        if len(embedding_keys) > 0:
            ...
            sc.pp.neighbors(
                adata,
                use_rep=use_rep,
                key_added=(neighbors_key if neighbors_key != "neighbors" else None),
            )
        else:
    ...
    if connectivities is None:
        raise ValueError(
            f"Connectivity matrix not found for key '{neighbors_key}' in adata.uns or adata.obsp"
        )
    ...
    if label_key:
        ...
        for cat in labels.cat.categories:
            ...
            n_components, component_labels = connected_components(
                csgraph=sub_conn, directed=False, connection="strong"
            )
