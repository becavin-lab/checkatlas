...
def hist_external(adata,
                  partition_keys: Collection[str],
                  obsm_representation : Collection[str] = None):
    metrics_dict = dict(Davies_Bouldin=compute.davies_bouldin,
        ...
        for key in partition_keys:
            ...
            if obsm_representation:
                for rep in obsm_representation:
                    score = func(adata, partition_key=key, obsm_representation=rep)
                    method_representation = f'{key}_{rep}'
                    metric = name
                    to_plot_array.append([score, method_representation, metric])
        to_plot_df = pd.DataFrame(to_plot_array,
                                  columns=['score', 'clustering_representation', 'metric'])
        sns.barplot(x='metric', y='score', hue='clustering_representation', ax=plt.gca(),
                    data=to_plot_df)
        i += 1


def hist_internal(adata,
                  partition_keys: Collection[str],
                  reference):
    metrics_dict = dict(rand=compute.rand,
    ...
    for key in partition_keys:
        for name, func in metrics_dict.items():
            ...
            i += 1
    to_plot_df = pd.DataFrame(to_plot_array, columns=['score', 'clustering', 'metric'])
