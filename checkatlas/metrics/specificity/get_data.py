...
def mean_celltype(adata: anndata,
    ...
    if add_adata:
        adata.varm[f'ave_celltype_counts_{partition_key}'] = np.array(average_by_celltype.transpose())
        adata.uns[f'ave_celltype_index_{partition_key}'] = average_by_celltype.index
    return average_by_celltype


def get_average_celltype_counts(adata,
                                partition_key: str = "CellType"):
    """
    ...
    try:
        ...
        average_by_celltype = mean_celltype(adata,
...
def get_spe(adata,
            spe_metric: Literal['shannon', 'tau', 'gini'],
            partition_key):
    specs = {'shannon': compute.shannon_average,
