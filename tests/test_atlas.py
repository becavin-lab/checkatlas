...
@given("atlas_path", [(datasets.ADATA_TEST_PATH)])
def test_viable_obm(atlas_path): 
    adata = atlas.read_atlas(atlas_path)
...
def test_qc_table(atlas_path):
    ...
    args = parser.parse_args(['--path',checkatlas_path,
            ...
            "n_genes_by_counts",
            "pct_counts_mt",
        ],'--plot_celllimit','10000'])
    folders.checkatlas_folders(checkatlas_path)
    atlas.create_qc_tables(adata, atlas_path, args)
    atlas_name = checkatlas.get_atlas_name(atlas_path)
    csv_path = files.get_file_path(atlas_name, folders.QC,
...
def test_qc_plots(atlas_path):
    ...
    atlas_name = checkatlas.get_atlas_name(atlas_path)
    csv_path = files.get_file_path(atlas_name, folders.QC_FIG,
                                   checkatlas.QC_FIG_EXTENSION,
                                   args.path)
    assert os.path.exists(csv_path)
...
def test_umap_plots(atlas_path):
    ...
    atlas_name = checkatlas.get_atlas_name(atlas_path)
    csv_path = files.get_file_path(atlas_name, folders.UMAP,
                                   checkatlas.UMAP_EXTENSION,
                                   args.path)
    assert os.path.exists(csv_path)
...
def test_tsne_plots(atlas_path):
    ...
    atlas_name = checkatlas.get_atlas_name(atlas_path)
    csv_path = files.get_file_path(atlas_name, folders.TSNE,
                                   checkatlas.TSNE_EXTENSION,
                                   args.path)
    assert os.path.exists(csv_path)
